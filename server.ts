#!/usr/bin/env bun
/**
 * Discord channel — MCP bridge.
 *
 * Self-contained MCP server with full access control: pairing, allowlists,
 * guild-channel support with mention-triggering. State lives in
 * $DISCORD_STATE_DIR (default ~/.claude/channels/discord)/access.json —
 * managed by the /discord:access skill when used with Claude Code.
 *
 * Discord's search API isn't exposed to bots — fetch_messages is the only
 * lookback, and the instructions tell the model this.
 *
 * Fork modifications (Apache-2.0, see NOTICE):
 *   - DISCORD_TMUX_TARGET env: inject inbound messages as keystrokes into a
 *     named tmux session. Lets CLIs that don't subscribe to MCP server-push
 *     notifications (e.g. OpenAI Codex CLI) still react to Discord messages
 *     when run as a tmux daemon.
 */
...
const TMUX_TARGET = process.env.DISCORD_TMUX_TARGET

// Inject Discord inbound text as keystrokes into a tmux session running an
// interactive AI CLI (Codex, etc). Uses send-keys -l (literal) so payload
// content is never interpreted as tmux key names like "C-c". The payload
// reaches the CLI as one Enter-submitted prompt — the AI's own behavior
// protocol (e.g. AGENTS.md) decides whether to actually reply.
function tmuxInject(content: string, meta: Record<string, string>): void {
  process.stderr.write(`discord channel: tmuxInject entered TMUX_TARGET=${TMUX_TARGET ?? '(unset)'} contentLen=${content?.length ?? 0}\n`)
  if (!TMUX_TARGET) return
  if (!content) return
  // Wrap inbound in a tag the target AI can parse; mirrors the MCP
  // notification meta so behavior protocols can key on the same shape.
  const tagAttrs = Object.entries(meta)
    .map(([k, v]) => `${k}="${String(v).replace(/"/g, '&quot;')}"`)
    .join(' ')
  const payload = `<channel source="discord" ${tagAttrs}>\n${content}\n</channel>\n`
  try {
    const sendText = spawn('tmux', ['send-keys', '-t', TMUX_TARGET, '-l', '--', payload], { stdio: 'ignore' })
    sendText.on('exit', code => {
      if (code !== 0) {
        process.stderr.write(`discord channel: tmux send-keys text exit ${code}\n`)
        return
      }
      const sendEnter = spawn('tmux', ['send-keys', '-t', TMUX_TARGET, 'Enter'], { stdio: 'ignore' })
      sendEnter.on('error', err => process.stderr.write(`discord channel: tmux send-keys Enter failed: ${err}\n`))
    })
    sendText.on('error', err => process.stderr.write(`discord channel: tmux inject failed: ${err}\n`))
  } catch (err) {
    process.stderr.write(`discord channel: tmux inject threw: ${err}\n`)
  }
}
...
client.on("messageCreate", msg => {
  process.stderr.write(`discord channel: messageCreate ch=${msg.channelId} user=${msg.author.username} bot=${msg.author.bot} contentLen=${msg.content?.length ?? 0}\n`)
  // Don't reply to ourselves (prevents self-echo loop). Other bots are
  // intentionally allowed through — the AI's behavior protocol
  // (AGENTS.md / CLAUDE.md) decides whether to actually reply, so multi-AI
  // groups can collaborate without the plugin filtering them out.
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  process.stderr.write(`discord channel: gate action=${result.action} channelType=${msg.channel.type} for user=${msg.author.username}\n`)
  ...
  // Fork addition: if DISCORD_TMUX_TARGET is set, also inject as keystrokes
  // for AI CLIs that don't subscribe to MCP server-push notifications.
  tmuxInject(content, meta)
}
